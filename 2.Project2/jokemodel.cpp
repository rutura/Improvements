...
{
  if (index.row() < 0 || index.row() >= m_datasource->dataItems().count()) { return QVariant(); }
  Joke *joke = m_datasource->dataItems().at(index.row());
  if (role == JokeDataRole) { return joke->joke(); }
  return QVariant();
}
...
DataSource *JokeModel::datasource() const { return m_datasource; }

void JokeModel::setDatasource(DataSource *datasource)
{
  beginResetModel();

  if (m_datasource && signalsConnected) m_datasource->disconnect(this);
  ...
  connect(m_datasource, &DataSource::postItemAdded, this, [=]() { endInsertRows(); });

  connect(
    m_datasource, &DataSource::preItemRemoved, this, [=](int index) { beginRemoveRows(QModelIndex(), index, index); });

  connect(m_datasource, &DataSource::postItemRemoved, this, [=]() { endRemoveRows(); });
